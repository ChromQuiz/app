...
> （**P2-e2 / P2-e3 は当時ラベル未記載**のため、下記の段階区分は commit 証拠からの再構成）。
...
### P2-e2 — dual-write ＋ 既存行 backfill
- **実施**: `create-entry` / `admin-create-entry` が pepper 化 v2 を生成し、旧列と v2 を**両方書込（dual-write）**。
  既存行の v2 を一度だけ埋める内部 backfill 関数を導入・実行し、完了後に撤去。
- **完了日**: 2026-07-12〜14（commit 日付に準ずる）
- **commit**: `62a1e2e` (Dual-write participant hash v2 on entry creation) /
  `9f895bb` (Add temporary participant hash backfill function) /
  `2e307ee` (Allow one-time internal participant hash backfill) /
  `9c75e09` (Remove temporary participant hash backfill function)
- **deploy**: `create-entry` / `admin-create-entry`（dual-write）
- **検証**: 全行で v2 両列が非 NULL＝backfill 完遂（状態証跡：現在 v2_null=0）
- **注**: フェーズラベルは当時未記載。commit 証拠から再構成。

### P2-e3〜P2-e4 — v2 での認証（dual-read → v2 優先）＋ id 経路キャンセル
- **実施**: `_shared/participant_auth.ts` を導入し、credential 認証を v2 で照合（当初は旧列 fallback つき）。
  0 件と DB エラーを区別する v2 lookup の堅牢化。認証済み entry を id で扱う `cancel_entry_by_id_atomic` を追加し、
...
- **注**: e3（dual-read）と e4（v2 認証確立＋id キャンセル）の境界ラベルは再構成。migration は "P2-e4" と明記。
...
| # | 項目 | 種別 | 優先 | 根拠 |
|---|------|------|------|------|
| 1 | RLS ＋ 公開/認証 Edge Function の回帰テストスイート整備 | 独立残課題 | 高 | 実施済みハードニングの退行を守る土台がない |
| 2 | `entries` 機密列（encrypted_pii / v2 hash）の列レベル制限の回帰テスト | 独立残課題 | 高 | 機密列漏洩の自動検知がない |
| 3 | 全公開/認証データフローの一括セキュリティレビュー（RLS・grant・policy・client 経路） | 独立残課題 | 中 | roadmap In Progress。断片的レビューのみ |
| 4 | `public_entry_list` 公開範囲の妥当性レビュー（PII 非保持前提の再確認） | 独立残課題 | 中 | 公開範囲を広げた変更の継続監査 |
...
## 付記
- **P2-e2 / P2-e3 のフェーズ境界ラベルは当時コードに未記載**のため、本文書は commit/migration の実証跡から再構成した。
